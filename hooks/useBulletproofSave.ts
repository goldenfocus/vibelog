...
export function useBulletproofSave() {
  ...
  const trackSaveMetrics = useCallback((event: string, data?: any) => {
    try {
      ...
      switch (event) {
        ...
        case 'save_success':
          ...
          if (data?.responseTime) {
            ...
            metrics.summary.averageResponseTime = (currentAvg * (count - 1) + data.responseTime) / count;
          }
          break;
        case 'save_failed':
      ...
      if (event === 'save_success' || event === 'save_failed') {
        const successRate = (metrics.summary.successfulSaves / metrics.summary.totalSaves * 100).toFixed(1);
        console.log(`📊 [SAVE-METRICS] Success Rate: ${successRate}%, Avg Response: ${metrics.summary.averageResponseTime.toFixed(0)}ms, Local Backups: ${metrics.summary.localBackups}`);
      }
    } catch (error) {
  ...
  const storeLocalBackup = useCallback((data: SaveVibelogData) => {
    try {
      ...
      trackSaveMetrics('local_backup_created', { backupId: backup.id, contentLength: data.content.length });
      return backup.id;
    } catch (error) {
  ...
  const performSave = useCallback(async (
    ...
    try {
      ...
      if (result.success) {
        ...
        console.warn('⚠️ [BULLETPROOF-SAVE] Server reported issues but may have captured data:', result);
      ...
      if (attempt < MAX_RETRY_ATTEMPTS) {
        ...
        trackSaveMetrics('save_failed', {
          ...
          attempts: attempt,
          finalError: networkError instanceof Error ? networkError.message : 'Unknown network error',
          backupId
        });
  ...
  }, [extractTitle, getSessionId, markBackupComplete, trackSaveMetrics]);

  // Main save function that never fails
  const saveVibelog = useCallback(async (data: SaveVibelogData): Promise<SaveResult> => {
    ...
    try {
      ...
      return {
        ...
        healthScore: metrics.summary ?
          Math.round((metrics.summary.successfulSaves / Math.max(1, metrics.summary.totalSaves)) * 100) : 100
      };
    } catch (error) {
      console.error('❌ [SAVE-METRICS] Failed to get metrics:', error);
      return {
        summary: { totalSaves: 0, successfulSaves: 0, failedSaves: 0, localBackups: 0, retryAttempts: 0, averageResponseTime: 0 },
        recentEvents: [],
        healthScore: 100
      };
    }
  ...
  const retryPendingBackups = useCallback(async () => {
    ...
    for (const backup of pendingBackups.slice(0, 5)) { // Limit to 5 at a time
      try {
