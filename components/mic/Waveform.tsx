...
export default function Waveform({ 
  ...
  const containerClasses = variant === 'recording' 
  ...
  return (
    <div className={`${containerClasses} ${className}`}>
      {levels.map((lvl, i) => {
        ...
        const lightness = (variant === 'recording' ? 55 : 60) + lvl * (variant === 'recording' ? 25 : 20);
        ...
        return (
          <div
            ...
            style={{
              ...
              width: variant === 'recording' 
                ? (typeof window !== 'undefined' && window.innerWidth < 640 ? "6px" : "9px")
                : "6px",
              backgroundColor: barColor,
              borderRadius: variant === 'recording' ? "2.5px" : "3px",
              boxShadow: `0 0 ${lvl * (variant === 'recording' ? 
                (typeof window !== 'undefined' && window.innerWidth < 640 ? 8 : 12) : 8)}px ${barColor}`,
              opacity: variant === 'recording' ? (0.8 + (lvl * 0.2)) : (0.7 + (lvl * 0.3)),
              transform: variant === 'recording' 
