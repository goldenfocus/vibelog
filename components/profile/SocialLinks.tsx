...
export function SocialLinks({ profile, variant = 'default', animated = true }: SocialLinksProps) {
  ...
  const sizeClasses = variant === 'large' ? 'w-12 h-12 text-lg' : 'w-10 h-10 text-base';
...
function SocialLinkButton({ link, index, sizeClasses, animated }: SocialLinkButtonProps) {
  ...
  return (
    <a
      ...
      className={` ${sizeClasses} bg-surface/50 flex items-center justify-center rounded-full border border-border/50 text-muted-foreground backdrop-blur-sm transition-all duration-300 ${link.color} hover:bg-surface/80 hover:scale-110 hover:border-electric/30 hover:shadow-lg hover:shadow-electric/20 active:scale-95 ${animated ? 'animate-fadeInUp' : ''} group`}
      style={{
