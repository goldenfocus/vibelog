...
import { Play, Pause, Music2, Video } from 'lucide-react';

import { cn } from '@/lib/utils';

interface InteractionLayerProps {
  vibelogId?: string; // Optional - not currently used but may be needed for future features
  hasAudio?: boolean;
  hasVideo?: boolean;
  isHovered?: boolean;
  isPlaying?: boolean;
    ...
    <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
      ...
      <button
        ...
        className={cn(
          ...
          isHovered ? 'translate-y-0 scale-100 opacity-100' : 'translate-y-4 scale-95 opacity-0'
        )}
      ...
      >
        ...
        {/* Play/Pause icon */}
        {isPlaying ? (
          <Pause className="h-7 w-7 fill-white text-white transition-transform group-hover/play:scale-110" />
        ) : (
      ...
      >
        ...
        <span className="text-xs font-medium text-white/90">{hasVideo ? 'Video' : 'Audio'}</span>
      </div>
    </div>
  );
}
