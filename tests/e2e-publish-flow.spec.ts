...
test.describe('E2E Publish Actions Flow', () => {
  ...
  test('should complete full transcript complete → publish → copy flow', async ({ page }) => {
    ...
    expect(copiedText).toContain('This is a medium-length vibelog content');
  ...
  test('should handle copy with signature functionality', async ({ page }) => {
    ...
    const copiedText = await page.evaluate(() => (window as any).lastCopiedText);
    expect(copiedText).toContain('Vibelog content that will include a signature when copied.');
  ...
  test('should handle long content correctly', async ({ page }) => {
    ...
    const copiedText = await page.evaluate(() => (window as any).lastCopiedText);
    expect(copiedText).toContain('This is a much longer vibelog content');
  ...
  test('should handle full record → processing → publish flow with ProcessingAnimation', async ({
    page,
  }) => {
  ...
  test('should preserve Star Wars crawl effect visual consistency', async ({ page }) => {
    ...
    await expect(processingPanel).toHaveScreenshot('star-wars-crawl-preservation.png', {
      threshold: 0.1,
    });
  });
  ...
  test('should maintain responsive behavior of ProcessingAnimation', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      await expect(processingPanel).toHaveScreenshot(`processing-responsive-${viewport.name}.png`, {
        threshold: 0.1,
      });
    }
  });
  ...
  test('should handle ProcessingAnimation error states gracefully', async ({ page }) => {
    ...
    // Test vibelog generation error state
    const vibelogErrorPanel = page.getByTestId('processing-vibelog-generation-error');
    await expect(vibelogErrorPanel).toBeVisible();

    // Should still show the animation despite errors
    await expect(vibelogErrorPanel.getByText('⚡ Vibelogging your content...')).toBeVisible();
    await expect(vibelogErrorPanel.locator('.star-wars-crawl')).toBeVisible();
    ...
    await expect(transcriptionErrorPanel).toHaveScreenshot('processing-transcription-error.png', {
      threshold: 0.1,
    });
    await expect(vibelogErrorPanel).toHaveScreenshot('processing-vibelog-error.png', {
      threshold: 0.1,
    });
    await expect(bothErrorsPanel).toHaveScreenshot('processing-both-errors.png', {
      threshold: 0.1,
    });
  });
});
