...
export class HumorModule {
  ...
  detectSarcasm(vibe: VibeAnalysis): {
    ...
    if (sarcasmScore < 20) {
      level = 'none';
    } else if (sarcasmScore < 40) {
      level = 'light';
    } else if (sarcasmScore < 60) {
      level = 'moderate';
    } else if (sarcasmScore < 80) {
      level = 'heavy';
    } else {
      level = 'nuclear';
    }
  ...
  checkNotOkayButOkay(vibe: VibeAnalysis): {
    ...
    if (level > 80) {
      ...
      spiritualGrandmaWarning =
        "Sweetie, I can see through that smile. Your energy is telling a different story. Take a deep breath, drink some tea, and remember: it's okay to not be okay. The universe doesn't need you to be perfect. 💜";
    } else if (level > 60) {
      message = "You're trying hard to be okay, but the vibe says otherwise.";
      spiritualGrandmaWarning =
        "Darling, I sense you're carrying more than you need to. Sometimes the bravest thing is admitting you're not fine. The stars are listening. ✨";
    } else if (level > 40) {
      message = 'Slight "I\'m fine" energy detected.';
      spiritualGrandmaWarning =
        "Honey, even the moon has phases. It's natural to have ups and downs. Be gentle with yourself. 🌙";
    } else {
