...
export async function POST(request: NextRequest) {
  try {
    ...
    // Anonymous users are now allowed to clone voices
    // Their voice samples will be stored but not associated with a profile
    ...
    const recommendedMinSize = config.voiceCloning.minBytes; // configurable threshold
    ...
    if (usesElevenLabs) {
      try {
        ...
        elevenLabsForm.append('description', `Voice clone generated by VibeLog for user ${userId}`);
        ...
        const elevenResponse = await fetch(`${config.ai.elevenlabs.apiUrl}/voices/add`, {
          method: 'POST',
          headers: {
        ...
        if (!elevenResponse.ok) {
          const message =
            ...
            `Voice cloning failed with status ${elevenResponse.status}`;
        ...
        const resolvedVoiceId = payload?.voice_id || payload?.voiceId || payload?.id;
        if (!resolvedVoiceId || typeof resolvedVoiceId !== 'string') {
          console.error('❌ [ElevenLabs] Response missing voice_id:', payload);
          return NextResponse.json(
            {
              error: 'Voice cloning failed',
              message: 'ElevenLabs did not return a voice ID.',
            },
            { status: 502 }
          );
        }
    ...
    if (!voiceId) {
      voiceId = randomUUID();
      console.log('🎤 [VOICE-CLONE] Using internal voice ID for Modal/Coqui:', voiceId);
    }
    ...
    // For anonymous users, store under voices/anonymous/{voiceId}
    const voiceStoragePath = userId
      ? `voices/${userId}/voice_sample.wav`
      : `voices/anonymous/${voiceId}/voice_sample.wav`;
    ...
    console.log('✅ [VOICE-CLONE] Voice cloned successfully:', {
      voiceId,
      voiceProvider,
    ...
    // Store voice clone ID in database (only for logged-in users)
    ...
    // Anonymous users skip this step since they don't have a profile
    if (userId) {
      try {
      ...
      console.log('ℹ️ [VOICE-CLONE] Anonymous user - skipping profile update');
    }
