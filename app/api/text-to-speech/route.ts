...
export async function POST(request: NextRequest) {
  try {
    ...
    // Initialize adminSupabase for cache operations
    ...
    const contentHash = hashTTSContent(cacheKey, voice);
      ...
      if (vibelogId && cachedEntry.audio_url) {
        try {
          ...
          if (vibelog && !vibelog.audio_url) {
            ...
            console.log(`💾 [TTS] Saved cached audio_url to vibelog ${vibelogId}`);
          }
        } catch (error) {
    ...
    let ttsService: 'openai' | 'mock' = 'openai'; // Track which service was used

    // Generate TTS using OpenAI (always use default voice)
      ...
      if (vibelogId && audioUrl) {
        try {
          ...
          if (vibelog && !vibelog.audio_url) {
            ...
            console.log(`💾 [TTS] Saved audio_url to vibelog ${vibelogId}`);
          }
        } catch (error) {
          // Don't fail the request if saving to vibelog fails
    ...
    // At this point, audioBuffer is guaranteed to be assigned (from OpenAI)
    if (!audioBuffer) {
